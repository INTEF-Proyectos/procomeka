...
describe("processElpxUpload with real fixtures", { timeout: 60_000 }, () => {
	...
	for (const fixture of FIXTURES) {
		...
		it(`processes "${fixture}" successfully`, { timeout: 60_000 }, async () => {
			...
			// Hash must be 40 hex chars (SHA-1)
...
describe("metadata content validation", { timeout: 30_000 }, () => {
	...
	it('"Manual de eXeLearning 3.0.elpx" has title', async () => {
		const fixturePath = path.join(FIXTURES_DIR, "Manual de eXeLearning 3.0.elpx");
		const metadata = await parseElpxMetadata(fixturePath);

		expect(metadata.title.length).toBeGreaterThan(0);
		console.log("[Manual] title:", metadata.title);
	});

	it("language field contains a reasonable value when present", async () => {
	...
	it("license field is populated when present", async () => {
...
describe("elpx content proxy integration", { timeout: 60_000 }, () => {
	...
	it("serves a second fixture through the proxy", async () => {
		// Process todos-los-idevices to test a richer fixture
		const fixturePath = path.join(FIXTURES_DIR, "todos-los-idevices.elpx");
		const result = await processElpxUpload(fixturePath, integrationBaseDir);
